 ...
 */
export const getParametersFromSchema = (schemaItem, functionName) => {
    if (!schemaItem) {
        return undefined;
    }
    const propertyName = `NamedParameters<typeof ${functionName}>`;
    const namedParameters = getSchema(schemaItem.schema?.definitions?.[propertyName]);
    ...
    const parameters = Object.keys(properties)
        ...
        const propertySchemaWithoutStandardContext = name === "context"
        ...
        const functionParameter = propertySchemaWithoutStandardContext
            ? {
                name,
                schema: propertySchemaWithoutStandardContext,
                simplifiedSchema: simplifySchema(name, propertySchemaWithoutStandardContext, getPossibleRefs(schemaItem), []),
                required,
            }
            : undefined;
...
//# sourceMappingURL=getParametersFromSchema.js.map