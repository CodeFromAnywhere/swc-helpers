 ...
 */
export const findTypescriptFiles = async (operations, absoluteFileOrFolderPaths) => {
    ...
    const typescriptFiles = (await Promise.all(basePath.map(async (absoluteFileOrFolderPath) => {
        if (!fs.existsSync(absoluteFileOrFolderPath))
            return;
        // folders
        if ((await fs.stat(absoluteFileOrFolderPath)).isDirectory()) {
            return (await getFolderTypescriptFiles(absoluteFileOrFolderPath, false)).map((x) => x.path);
        }
        // files
...
//# sourceMappingURL=findTypescriptFiles.js.map