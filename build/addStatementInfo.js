...
export const addStatementInfo = async (item, absoluteOperationPathsInScope, operationClassificationObject, operationSchemaObject) => {
    ...
    if (item.isExported && isRequireEnabled) {
        ...
        const file = isImportable && item.absolutePath && isImportRequired
        ...
        if (isImportable && item.modelName === "SwcFunction") {
            ...
            fullSwcStatement.config = file?.[item.name]?.config;
        ...
        if (isImportable && item.modelName === "SwcVariable") {
            ...
            if (canStringify) {
                ...
                fullSwcStatement.statement = file?.[item.name];
            }
        }
    }
    //NB: if it's a function, add parameters and namedParameters here.
    if (fullSwcStatement.modelName === "SwcFunction") {
        const fullSwcFunction = fullSwcStatement;
        const responseInterfaceName = `${pascalCase(fullSwcFunction.name)}Response`;
        const responseDefinitions = schemaItems?.find((x) => x.name === responseInterfaceName)?.schema?.definitions;
        ...
        const namedParametersItem = schemaItems?.find((x) => x.name === namedParametersName);
        ...
        fullSwcFunction.parameters = getParametersFromSchema(namedParametersItem, fullSwcFunction.name);
        return fullSwcFunction;
    }
    // NB: add typeinfo under type for interfaces
    if (fullSwcStatement.modelName === "SwcInterface") {
        const fullSwcInterface = fullSwcStatement;
        const schemaItem = schemaItems?.find((x) => x.name === fullSwcInterface.name);
        const typeDefinition = getSchema(schemaItem?.schema?.definitions?.[fullSwcInterface.name]);
        const simplifiedSchema = schemaItem && typeDefinition
            ? simplifySchema(fullSwcInterface.name, typeDefinition, getPossibleRefs(schemaItem), [])
            : undefined;
        const typeInfo = {
            isArray: false,
            isEnum: false,
...
//# sourceMappingURL=addStatementInfo.js.map