...
*/
export const regenerateImportsEverywhere = async (operations, basePath, isDryrun) => {
    ...
    const updateImport = (item) => {
        ...
        const statementsForThisImport = realStatements.filter((x) => x.name === item.name);
        ...
        if (!statementForThisImport.absolutePath ||
            !statementForThisImport.projectRelativePath) {
            // shouldn't happen
            console.log("ERR: got no paths for statement");
            return item;
        }
        const packageNameOfStatement = getObjectKeysArray(operations).find((key) => statementForThisImport.projectRelativePath.startsWith(`${operations[key]}/`));
        const packageNameOfImport = getObjectKeysArray(operations).find((key) => item.projectRelativePath.startsWith(`${operations[key]}/`));
        ...
        const relativePath = `${withoutExtension(getRelativeLinkPath(item.absolutePath, statementForThisImport.absolutePath))}.js`;
        const module = isAbsolute ? packageNameOfStatement : relativePath;
        if (module === item.module) {
            // nothing changed
            return item;
        }
        const newImport = { ...item, module, isAbsolute };
        console.log(item.projectRelativePath, `import { ${item.name} } from "${item.module}" -> "${module}"`);
        return newImport;
    };
    const result = await regenerateImports(operations, basePath, updateImport, isDryrun);
    const updatedAmount = result?.map((x) => x.needsUpdate).filter((x) => !!x).length || 0;
    return {
        updatedAmount,
...
//# sourceMappingURL=regenerateImportsEverywhere.js.map