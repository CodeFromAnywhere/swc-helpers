...
export const exportedArrowFunction = async () => {
    ...
    const res = await Promise.all(paths.map((p) => {
        return getTypescriptFileData({}, p);
    }));
    console.dir(res.map((x) => x.statements), { depth: 4 });
};
...
// );
//# sourceMappingURL=getTypescriptFileData.test.js.map