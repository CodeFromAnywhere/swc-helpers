...
import { createMappedKeysObject, createPromisedMappedKeysObject, makeArray, notEmpty, onlyUnique2, } from "from-anywhere";
 ...
 */
export const getStatements = async (operations, filter) => {
    ...
    const absoluteOperationPathsInScope = (await Promise.all(projectRelativeBasePaths.map((projectRelativeBasePath) => {
        return findOperationsInScope(path.join(projectRoot, projectRelativeBasePath));
    })))
        .filter(notEmpty)
        .flat()
        .filter((x) => {
        ...
        .filter(onlyUnique2());
    const absoluteBasePaths = projectRelativeBasePaths.map((p) => path.join(projectRoot, p));
    ...
    if (filter?.isCacheDisabled) {
        ...
        const operationClassificationObject = createMappedKeysObject(absoluteOperationPathsInScope, (key) => getOperationClassification(key));
        ...
        const operationSchemaObject = await createPromisedMappedKeysObject(absoluteOperationPathsInScope, async (absoluteOperationPath) => {
            const absoluteSchemaFilePath = path.join(absoluteOperationPath, "schema.json");
            const schemaItemsString = fs.existsSync(absoluteSchemaFilePath)
                ? await fs.readTextFile(absoluteSchemaFilePath)
                : undefined;
            const schemaItems = schemaItemsString
                ? tryParseJson(schemaItemsString) || undefined
                : undefined;
            return schemaItems;
        });
        ...
        const finalImports = imports
            .map((item) => {
            const operationPath = absoluteOperationPathsInScope.find((p) => item.absolutePath?.startsWith(`${p}/`));
            if (!operationPath) {
                //shouldn't happen
                return;
            }
            const operationClassification = operationClassificationObject[operationPath];
            const finalSwcImport = {
                ...item,
            ...
            .filter(notEmpty);
        const finalStatements = (await Promise.all(statements.map((item) => addStatementInfo(item, absoluteOperationPathsInScope, operationClassificationObject, operationSchemaObject)))).filter(notEmpty);
    ...
    const result = await Promise.all(absoluteOperationPathsInScope.map(async (absoluteOperationPath) => {
        const statementsPath = path.join(absoluteOperationPath, "statements.json");
        const importsPath = path.join(absoluteOperationPath, "imports.json");
        const statements = await readJsonFile(statementsPath);
        const imports = await readJsonFile(importsPath);
    ...
    if (notFoundAbsoluteOperationPaths.length > 0) {
        console.log(`Not found operations: ${notFoundAbsoluteOperationPaths.length} (Going to get them now without cache)`, notFoundAbsoluteOperationPaths);
    }
    // if not found, get it parsed.
    const projectRelativeBasePath = notFoundAbsoluteOperationPaths.map((x) => makeRelative(x, projectRoot));
    // console.log({ projectRelativeBasePath });
    const notFoundResult = notFoundAbsoluteOperationPaths.length > 0
