 ...
 */
export const findCircularDependency = async (startOperationName, operations) => {
    const dependenciesPerOperation = await getDependenciesPerOperation(Object.keys(operations), operations);
    const dependenciesPerOperationObject = mergeObjectsArray(dependenciesPerOperation.map((x) => ({
        [x.operationName]: x.dependencies || [],
    })));
    const dependencies = dependenciesPerOperationObject[startOperationName];
    if (!dependencies) {
...
export const findCircularDependencyRecursive = (dependenciesPerOperationObject, 
/**
...
stack) => {
    ...
    const dependencies = dependenciesPerOperationObject[last];
    if (!dependencies) {
...
//# sourceMappingURL=findCircularDependency.js.map