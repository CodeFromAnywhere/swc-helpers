...
export const statementNoSpecialFileFilter = (x) => {
    if (!x.absolutePath) {
        return false;
    }
    if ([
...
export const findDuplicateNames = async (operations) => {
    ...
    console.log({ loc });
    const statementsWithoutSpecialFiles = statements.filter(statementNoSpecialFileFilter);
    const duplicates = statementsWithoutSpecialFiles
        .filter(onlyDuplicates((a, b) => a.name === b.name))
        .map((x) => x.name)
        .filter(onlyUnique)
        .filter((x) => !["getStaticProps", "getStaticPaths"].includes(x))
        .map((name) => {
        const theseStatements = statementsWithoutSpecialFiles.filter((x) => x.name === name);
        return {
            name,
...
//# sourceMappingURL=findDuplicateNames.js.map