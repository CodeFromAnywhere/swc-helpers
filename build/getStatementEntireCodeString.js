...
*/
export const getStatementEntireCodeString = async (operations, statementNames) => {
    const results = (await Promise.all(statementNames.map((statementName) => getStatementWithDependenciesRecursive(operations, statementName)))).filter(notEmpty);
    if (results.length === 0) {
        return;
    }
...
${code}`;
    ...
    const importsCode = imports
        .map((x) => `import ${x.isDefaultImport ? x.name : `{ ${x.name} }`} from "${x.module}";`)
        .join("\n");
...
//# sourceMappingURL=getStatementEntireCodeString.js.map