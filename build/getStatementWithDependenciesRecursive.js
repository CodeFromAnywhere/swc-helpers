...
*/
export const getStatementWithDependenciesRecursive = async (operations, statementName, already = []) => {
    ...
    //   } imported`,
    // );
    const results = (await Promise.all(result.importedStatementNames.map(async (x) => {
    ...
    const thisStatementCodes = mergeObjectsArray(result.statements.map((x) => ({ [x.name]: x.raw })));
...
//# sourceMappingURL=getStatementWithDependenciesRecursive.js.map