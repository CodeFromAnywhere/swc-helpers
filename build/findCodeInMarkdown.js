 ...
 */
export const findCodeInMarkdown = async (text, isJavascript) => {
    if (!text)
    ...
    const { isSuccessful, parseResult } = await trySwcParse(trimmed, isJavascript);
    if (isSuccessful && trimmed && parseResult) {
        return { code: trimmed, parseResult, isSuccessful: true };
    }
    return { isSuccessful: false };
};
//# sourceMappingURL=findCodeInMarkdown.js.map