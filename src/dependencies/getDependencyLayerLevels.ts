...
*/
export const getDependencyLayerLevels = async (
  operationNames: string[],
  removedOperationNames: string[],
  level: number,
  ...
  const dependenciesWithoutRemovedPerOperation = dependenciesPerOperation
    .map((item) => {
      ...
      const dependenciesWithoutRemoved = item.dependencies?.filter(
        (name) => !removedOperationNames.includes(name),
      );
  ...
  writeToAssets(
    __filename,
    dependenciesWithoutRemovedPerOperation,
    `dependenciesPerOperation-${level}.json`,
  );
  ...
  const newRemovedOperationNames = [...removedOperationNames].concat(
    operationsWithoutDependencies,
  );

  return await getDependencyLayerLevels(
    operationNamesLeft,
    newRemovedOperationNames,
    level + 1,
    dependenciesPerOperation,
    operations,
  );
